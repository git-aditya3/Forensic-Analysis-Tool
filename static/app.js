...
(async function init() {
  ...
  try {
    ...
    const analyticsReady = Boolean(health.capabilities?.opencv?.available);
    $('#api-label').textContent = analyticsReady ? 'Local engine + analytics online' : 'Local engine online · analytics runtime unavailable';
    await loadCases();
  } catch (error) {
    setApiState(false);
    toast(error.message, true);
